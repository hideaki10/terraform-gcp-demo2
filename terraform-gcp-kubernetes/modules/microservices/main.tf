...
resource "google_service_account_iam_member" "pod_default_is_workload_identity_user" {
  ...
  member             = "serviceAccount:${var.gke_project}.svc.id.goog[${kubrnetes_namespace.microservice.metdata[0].name}/${kubernetes_service_account.pod_default.metadata[0].name}]"
}


resource "kubernetes_role_binding" "service_admins_is_view" {
  for_each = toset(var.service_viewrs)
...
resource "kubernetes_role_bingding" "service_admins_is_admin" {
...
resource "google_artifact_registry_repositroy" "container" {
